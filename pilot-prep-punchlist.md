...
- [ ] Confirm Klarna actually renders as a payment option inside the embedded flow — blocked on a fully onboarded Stripe Connect account. My test club's Connect account isn't onboarded (no bank/business info), so session creation now correctly fails with `insufficient_capabilities_for_transfer` instead of the ui_mode error. Need either: (a) someone to finish my test club's Stripe onboarding link in a browser (test mode, fake data is fine, ~2 min), or (b) confirm "Team Richmond"/BBALL1 already has Stripe connected and get its club_admin login so I can test against it instead.
