...
- [ ] **Real bug found while verifying the above landed:** the `events` table's Supabase role only has `INSERT` granted, not `SELECT` — every `trackEvent()` call all session (screen views, checkout events, login/signup events) has been silently succeeding into a table nothing can ever read back from. Run in the Supabase SQL editor: `GRANT ALL ON public.events TO service_role;`
